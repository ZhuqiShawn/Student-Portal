...
public class TestPortal {
   ...
   public static void main(String[] args) {
      try{
         ...
         pause();
         // 2. Register the student for an unrestricted course, and check that they end up registered (print info again).
         ...
         System.out.println(c.register("1111111111", "CCC555")); 
         pause();
         // 4. Unregister the student from the course, and then unregister again from the same course. 
         // Check that the student is no longer registered and the second unregistration gives an error response.
         System.out.println(c.unregister("1111111111", "CCC555")); 
         pause();
         prettyPrint(c.getInfo("1111111111")); 
         pause();
         System.out.println(c.unregister("1111111111", "CCC555")); 
         pause();
         // 5. Register the student for a course that they don't have the prerequisites for, and check that an error
         // is generated. 
         System.out.println(c.register("1111111111", "CCC444")); 
         pause();
         // 6. Unregister a student from a restricted course that they are registered to, and which has at least 
         // two students in the queue. Register again to the same course and check that the student gets the 
         // correct (last) position in the waiting list.
         ...
         prettyPrint(c.getInfo("1111111111")); 
         pause();
         // 7. Unregister and re-register the same student for the same restricted course, and check that the student 
         // is first removed and then ends up in the same position as before (last).
         System.out.println(c.unregister("1111111111", "CCC222")); 
         prettyPrint(c.getInfo("1111111111")); 
         pause();
         System.out.println(c.register("1111111111", "CCC222"));
         prettyPrint(c.getInfo("1111111111")); 
         pause();
         // 8. Unregister a student from an overfull course, i.e. one with more students registered than there are 
         // places on the course (you need to set this situation up in the database directly). Check that no student 
         // was moved from the queue to being registered as a result.
         // course CCC222 is overfull now
         System.out.println(c.unregister("7777777777", "CCC444")); 
         prettyPrint(c.getInfo("8888888888")); 
         pause();
         // 9. Unregister with the SQL injection you introduced, causing all (or almost all?) registrations to disappear. 
         System.out.println(c.unregister_sql_injection("9999999999", "CCC222' OR 'a'='a")); 
